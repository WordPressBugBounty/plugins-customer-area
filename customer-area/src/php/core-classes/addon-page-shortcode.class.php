...
class CUAR_AddOnPageShortcode {
	...
	public function process_shortcode( $params = array(), $content = null ) {
		...
		if (
			(!is_user_logged_in() && $this->page_addon->get_slug() !== 'customer-login') ||
			!method_exists($this->page_addon, 'is_accessible_to_current_user') ||
			!$this->page_addon->is_accessible_to_current_user()
		) {
			return '<div class="cuar-message cuar-error">' . esc_html__('You are not allowed to view this content.', 'cuar') . '</div>';
		}
